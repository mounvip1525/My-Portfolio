...
export default function Projects() {
    ...
    const [projects1,setProjects1]=useState([
            ...
            },
            {
                ...
                link:"Google Forms clone with additional features like adding to folders, interactive UI, Hassle free form management",
            },
            {
                id:6,
                pic:musicplayer,
    ...
    const [techStack,setTechStatct]=useState({});
    const [sourceCode,setSourceCode]=useState("");
    const [launch,setLaunch]=useState("");
    let showModal1 = (name,description,img,link,code) => { 
        setName(name);
    ...
    return (
        <div className={styles.wrapper}>
            ...
            <Modal show={true} show={showModal} onHide={hideModal} name={name} description={description} img={img} link={launch} code={sourceCode} />
            <div className={styles.container}>
                {projects1.map((project)=>{
                return(
                    <div className={styles.project} onClick={()=>showModal1(project.name,project.description,project.pic,project.link,project.sourceCode)}>
            ...
            {projects2.map(project=>{
                return(
                    <div className={showClass} onClick={()=>showModal1(project.name,project.description,project.pic,project.link,project.sourceCode)}>
                        <img src={project.pic} alt="My project" />
                        <div className={styles.projectName}>
                            <p>{project.name}</p>
                        <div title="Launch Live"><a href={project.link}><LaunchIcon /></a></div>
